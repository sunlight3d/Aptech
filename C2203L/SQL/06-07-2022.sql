...
ON Result --on table's name
AFTER INSERT AS --event(when)
	DECLARE @Mark AS FLOAT --declare local variable
	SET @Mark = (SELECT TOP 1 Mark FROM INSERTED) 
	IF (@Mark < 0) 
	BEGIN 
		RAISERROR ('Cannot insert mark < 0',16,10);
		--16-severity(19-25), 20-state(0-255)
		ROLLBACK 
	END
SELECT * FROM Result;
--test trigger
...
ON Subject --on table's name
0x01
AFTER UPDATE AS --event(when)
	--check Whether SubjectName is "modified" ?
BEGIN 
	IF (COLUMNS_UPDATED() & 10) = 10 
	BEGIN
		RAISERROR ('You cannot update SubjectName',16,10);		
		ROLLBACK 
	END
END
...
UPDATE Subject SET SubjectName='C Programming 10' WHERE SubjectId=1;
/* Use IF (COLUMNS_UPDATED() & 10) = 10 to see whether all columns 2, 3,   
and 4 are updated. */  
