...
//        noteModify.insertNote(new Note("learn Java", false, new Date()));
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener(){
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int selectedPosition, long nodeId) {

                Cursor cursor = (Cursor) customAdapter.getItem(selectedPosition);
                Note selectedNote = Note.getNoteFromCursor(cursor);
                Intent intent = new Intent(MainActivity.this,
                        DetailActivity.class);
                intent.putExtra("selectedNote", (Serializable)selectedNote);
                startActivity(intent);
                Log.d(MainActivity.class.getName(), "click list item");
            }
        });
    ...
    public void updateNote(long nodeId, Note note) {
        this.noteModify.updateNote(nodeId, note);
        this.refreshListView();
    }
